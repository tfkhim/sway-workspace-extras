...
fn find_previous_workspace(tree: &Node) -> i32 {
    let workspaces = Workspaces::new(tree);
    workspaces.predecessor_of_focused()
}
...
impl<'a> Workspaces<'a> {
    ...
    fn predecessor_of_focused(&self) -> i32 {
        max(self.find_focused_workspace().unwrap_or(0) - 1, 1)
    }
    ...
    fn find_focused_workspace(&self) -> Option<i32> {
        self.nodes
            .iter()
            .find(|w| w.find_as_ref(|n| n.focused).is_some())
            .and_then(|w| w.num)
    }
    ...
    fn last_non_empty_workspace(&self) -> Option<i32> {
        self.nodes
            .iter()
            .filter(|w| !w.nodes.is_empty() || !w.floating_nodes.is_empty())
            .filter_map(|w| w.num)
            .max()
    }
}
